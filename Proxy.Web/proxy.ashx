...
public class proxy : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        //    response.StatusDescription = "Not a valid URI";
        //    response.Write("Not a valid URI");
        //    response.End();
        //    return;
        //}

        uri = uri.Replace("kabellaegningGrundkort", ConfigurationManager.AppSettings["kabellaegningGrundkort"]);
        uri = uri.Replace("kabellaegningFeatureSearch", ConfigurationManager.AppSettings["kabellaegningFeatureSearch"]);
        uri = uri.Replace("kabellaegningFeatureQuery", ConfigurationManager.AppSettings["kabellaegningFeatureQuery"]);        

        // Debug: Ping to make sure avaiable (ie. http://localhost/TestApp/proxy.ashx?ping)
        if (uri.EndsWith("ping"))
        {
            context.Response.Write("Hello proxy @ " + DateTime.Now.ToString("HH:mm:ss dd-MM-yyyy"));
            return;
        }
